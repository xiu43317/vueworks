    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <div class="collapse navbar-collapse" :class="{'show':show}" id="navbarSupportedContent">
          ...
          <form class="form-inline my-2 my-lg-0">
            <div class="form-inline my-2 my-lg-0">
              ...
              >
                ...
                <option
                  v-for="(option, index) in regionOption"
                  :value="option"
                  :key="index"
                >
                  {{ option }}
        ...
        <ul class="navbar-nav ml-auto">
          <li class="nav-item">
            <a
              class="nav-link hidden-xs-down"
              target="_blank"
              href="http://data.kcg.gov.tw/dataset/open1999"
              >資料出處：data.kcg.gov.tw</a
            >
          </li>
        </ul>
      </div>
    ...
    <div v-else>
      ...
      <div class="row">
        <div
          ...
          v-for="(item, index) in opendate"
          :key="index"
        >
          <div class="img-thumbnail">
            <span class="label bg-warning text-light">{{ item.ZipName_ }}</span>
            <span class="label bg-info text-light">{{ item.UnitName_ }}</span
            ><br />
            <span class="label label-ligth-pink text-light">{{
              item.InformDesc_
            }}</span>
            <div class="caption">
              <h3>
                <a :href="getGoogleMap(item.address_)" target="_blank">{{
                  item.address_
                }}</a>
              </h3>
